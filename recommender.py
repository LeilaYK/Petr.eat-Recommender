...
class Recommend_items:
    ...
    def json_to_corpus(self):
        self.df['BOW'] = (self.df['ingredient'] + self.df['tag_list']).apply(self.list2str)
        tokenized = ''
        for ingredient, tags in zip(self.df['ingredient'].values, self.df['tag_list'].values):
            item_token = ''
    ...
    def get_item_vector(self):
        ...
        df['BOW'] = (df['ingredient'] + df['tag_list']).apply(self.list2str)
    ...
    def recommend_ids(self, item_id, topn=20):
        similarity = {}
        target = item_id
...
#                 print('item',item)
#                 print('target',target)
                if (np.array(self.data[item]['vector']).reshape(1,-1).shape[1] != 1 and np.array(self.data[target]['vector']).reshape(1,-1).shape[1] != 1):
